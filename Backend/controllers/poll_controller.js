...
import sql from '../db.js';
import redis from '../redis.js'; // (Keep this if you go back to Redis later)
...
export const CreatePoll = async (req, res) => {
    ...
    try {
        ...
        while (!isUnique) {
            const check = await sql`SELECT id FROM polls WHERE secret_code = ${secretCode}`;
            if (check.length === 0) isUnique = true;
            else secretCode = generateSecretCode(); 
        }

        // postgres.js handles transactions beautifully with a callback function
        const { newPoll, newOptions } = await sql.begin(async (sql) => {
            // 1. Insert the Poll
            const pollResult = await sql`
                INSERT INTO polls (creator_id, question, secret_code, is_public, expires_at)
                VALUES (${req.user}, ${question}, ${secretCode}, ${isPublic || false}, ${expiresAt}) 
                RETURNING *
            `;
            
            const pollId = pollResult[0].id;

            // 2. Insert Options (postgres.js bulk inserts arrays of objects automatically!)
            ...
            const optionsResult = await sql`
                INSERT INTO poll_options ${sql(optionsData)} 
                RETURNING *
            `;

            return { newPoll: pollResult[0], newOptions: optionsResult };
        });
...
export const CastVote = async (req, res) => {
    try {
        ...
        const pollResult = await sql`
            SELECT id, question, expires_at, is_public 
            FROM polls WHERE secret_code = ${secretCode}
        `;
        ...
        res.json({
            ...
            options: optionsResult // No more .rows!
        });
...
// @route   POST /api/polls/:id/check-email
export const check_email = async (req, res) => {
    const { email } = req.body;
    const pollId = req.params.id;
    ...
    try {
        const voteCheck = await sql`
            ...
            WHERE poll_id = ${pollId} AND voter_email = ${email}
        `;

        res.json({ hasVoted: voteCheck.length > 0 });
    } catch (err) {
        console.error(err.message);
        res.status(500).json({ error: 'Server Error' });
    }
...
export const vote = async (req, res) => {
    ...
    if (!email || !optionId) return res.status(400).json({ error: 'Email and Option ID are required' });
...
// @route   GET /api/polls/public/active
export const GetPublicPolls = async (req, res) => {
    try {
        const now = new Date();
